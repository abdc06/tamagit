...
프로젝트는 **절대경로로 식별된다** — Claude Code 가 그 경로로 기록하기 때문이다. 폴더 이름을 바꾸면
같은 작업이 서로 다른 두 프로젝트가 되고, 프롬프트 수·보스전·"프로젝트 N곳"이 전부 쪼개진다.

```bash
tamagit projects                          # ✗ 는 디스크에 없는 경로
tamagit projects merge code-rpg tamagit   # 끝 폴더 이름만 줘도 된다
tamagit projects unmerge code-rpg         # 규칙만 해제, 이미 옮긴 기록은 그대로
```

`merge` 는 한 번의 `UPDATE` 가 아니라 **규칙을 남긴다.** 이후 모든 `sync` 가 그 규칙을
새로 들어오는 행과 이미 쌓인 행 양쪽에 적용하므로, 개명 직후까지 열려 있던 세션이 하루 뒤에
옛 경로로 기록을 남겨도 다시 쪼개지지 않는다.

자동 판정은 일부러 하지 않는다. 디스크에서 사라진 경로는 개명일 수도, 이동일 수도, 삭제일 수도 있다 —
구분할 수 있는 건 사용자뿐이라 표시만 하고 기다린다.
