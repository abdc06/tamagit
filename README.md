...
> 내 Claude Code 활동을 읽어, **코딩을 RPG로** 만든다.
...
`~/.claude/history.jsonl`은 **30일 뒤 사라진다**. 실측으로 확인했다 — 파일에 정확히 28.9일치만 남아 있었다.
...
실측 기준(2026-07-22, 3,666건): 필드는 `display` / `pastedContents` / `timestamp` / `project` / `sessionId`
다섯 개로 고정이고, 스키마 변형 0가지, 파싱 실패 0건. `timestamp`는 epoch **ms**이고 파일은 append-only 오름차순이다.
`(sessionId, ts)` 조합에 중복이 없어 멱등 적재 키로 쓴다.
...
길이 보상이 **로그 스케일**인 이유: 실측상 최다 입력이 `"진행시켜"`(4자·33회)이고 최장이 1,184자다.
선형이면 장문 하나가 하루를 끝내버리고, 균일하면 추임새와 설계 프롬프트가 같은 값이 된다.
현재 곡선은 4자 12 XP, 36자(p50) 18 XP, 1,184자 43 XP다.

레벨업에 필요한 XP는 `500 × 레벨^1.4`. 하루 130 프롬프트 기준 30일이면 Lv.13 언저리다.

### 하루 경계 — 새벽 4시

`03:59`의 활동은 **전날**로 귀속된다. 실측상 20~23시 활동이 전체의 21%라, 자정 경계를 쓰면
밤샘 코딩이 두 날로 쪼개지면서 스트릭 체감이 망가진다. `--day-start 0`으로 끌 수 있다.
...
세션을 통째로 쓰지 않는 이유: 실측 최장 세션이 1,473분(24.5시간)인데 프롬프트가 15개뿐이었다.
...
필드 조합이 스파이크 시점(2026-07-22)과 달라지면 `npm test`가 터진다 = 파서를 고치라는 신호다.
...
- **토큰/비용 = 마나** — `~/.claude/projects/**/*.jsonl`의 `message.usage`에 있다. 다만 그쪽은 **394MB**(1차 소스의 460배)라 증분 파싱이 필요하다.
- **툴 스킬트리** — 같은 파일에 툴 사용 빈도가 있다 (실측 샘플: Bash 671 > Edit 367 > Read 178 > Write 142).
- git 커밋 연동, 공유용 프로필 카드, 리더보드/클라우드 동기화.

## 알려진 한계

- **총 몰입 시간은 낙관적이다.** 유휴 30분 미만 간격은 전부 "활동 중"으로 계산하므로, 29분 자리를 비워도 몰입에 포함된다. 보스전 판정에는 건수 조건(15건)이 함께 걸려 있어 영향이 적지만, 상단 타일의 `287h` 같은 숫자는 상한으로 읽는 게 맞다.
